...
async function createTransactional({ userId, items }) {
  ...
  try {
    ...
    const orderRes = await client.query(
      `INSERT INTO orders(user_id, status, total, payment_status)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [userId, 'Pendiente', total.toFixed(2), 'pendiente']
    );
...
async function markAsPaid(id, { payment_method, payment_reference }) {
  const { rows } = await pool.query(
    `UPDATE orders
